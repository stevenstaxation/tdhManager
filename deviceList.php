...
if (mysqli_num_rows($result) != 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      <td class='text-center align-middle' style='padding:0 3px;'>" . $row['supplierName'] . "</td>
      ...
      <td class='text-center align-middle' style='padding:0 3px;'>" . $row['installerName'] . "</td>";
       ...
       <td class='text-center align-middle'><btn class='btn btn-sm btn-warning' onclick='showFullDevice(\"" . $row['ID'] . "device\")'><i class='bi bi-pencil-fill h5'></i></btn></td>";
        ...
        if ($row['deviceNote'] && $row['deviceNote'] != "") {
            $returnString .= "<td class='text-center align-middle'><btn class='btn btn-sm btn-warning' onclick='showDeviceNotes(\"" . $row['ID'] . "device\")'><i class='bi bi-journal-check h5'></i></btn></td>";
        } else {
      ...
      $('#deviceListTable').DataTable({
        ...
        columnDefs: [
          ...
          {'targets': 3, 'createdCell': function (td,cellData, rowData, row, col) {
           ...
           if (rowData[19]!=1 && rowData[8].includes('Installed') && rowData[12]=='Active') {
            ...
            {'targets': 7, 'createdCell': function (td,cellData, rowData, row, col) {
                ...
                if ((rowData[20]!=1) && (rowData[0]!='DHD' && rowData[0]!='DHINSTALL' && rowData[8].includes('Installed') && rowData[12]=='Active')) {
                    $(td).css('color', alertColour);
                }
                }
        },
            {'targets': 10, 'createdCell': function (td,cellData, rowData, row, col) {
            if (rowData[10]=='' || rowData[10]==null) {
                $(td).html('No SIM Phone assigned');
            }
            if ((rowData[21]!=1) && (rowData[0]!='DHD' && rowData[0]!='DHINSTALL' && rowData[8].includes('Installed') && rowData[12]=='Active')) {
                $(td).css('color', alertColour);
            }
        }
    },
