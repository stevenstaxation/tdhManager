...
if ($affectedRows>0) {
    if ($affectedRows>1) {
        $returnString .= "There are $affectedRows customers who are assigned to $brokerName. Each customer will have their broker reallocated as 'None selected'.<br>";
...
$anyContacts =  mysqli_num_rows($result);
    if ($anyContacts>1) {
        ...
        $returnString .= "There is $anyContacts contact attached to $brokerName. The contact will be lost if you continue with the deletion.";
    }
...
if ($returnString !="<div class='alert alert-danger'>") {
    ...
    <div id='hiddenIDToDelete' class='d-none'>" . $brokerNumber . "</div>";
