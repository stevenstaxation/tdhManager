...
<div id='deviceLongList' class='listHeader'>
  ...
  $returnString .= "<btn class='btn btn-warning' style='margin: 10px 10px;' id='showJobMap' onclick='showJobMap()'>Job Map</btn>";
...
$hideDatePassed = ($sqlFILTER & 3);
$hideAwaitApproval = ($sqlFILTER & 4);
$hideCompleted = ($sqlFILTER & 5);

echo $sqlFILTER;

if (isset($sqlFILTER) && $sqlFILTER!=-1) {
  $sql .= " WHERE (tblJobs.status <> '$hidePending' AND tblJobs.status <> '$hideBooked' AND tblJobs.status <> '$hideDatePassed' AND tblJobs.status <> '$hideAwaitApproval' AND tblJobs.status <> '$hideCompleted')";
} 
