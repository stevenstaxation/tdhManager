...
function updateEditInsurer() {
  ...
  dataToPost.insurerAddress2 = document.getElementById(
    "editInsurerAddress2"
  ).value;
  dataToPost.insurerAddress3 = document.getElementById(
    "editInsurerAddress3"
  ).value;
  dataToPost.insurerAddress4 = document.getElementById(
    "editInsurerAddress4"
  ).value;
          ...
          if (data.includes("deleted")) {
            ...
            $.ajax({
              ...
              success: function (data) {
                setTimeout(function () {
                  $("#devicesList").html(data);
                  $("#insurerNameSelection option:first").attr(
                    "selected",
                    "selected"
                  );
                  $("#insurerNameSelection").trigger("change");
  ...
  if (queryDelete == false) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.includes("success")) {
          $("#currentInsurerMessageBox").html(
            "<div class='alert alert-success'>Insurer deleted successfully</div>"
          );
