  ...
  Combobox.prototype = {
    ...
    template: function () {
      if (this.options.bsVersion == "2") {
        ...
        return (
          '<div class="combobox-container"> <input type="hidden" /> <div class="input-group"> <input type="text" autocomplete="off" />' +
          '<span class="input-group-append"' +
          (hasPopper ? ' data-toggle="dropdown" data-reference="parent"' : "") +
          ">" +
          '<span class="input-group-text dropdown-toggle' +
          (this.options.iconCaret ? " custom-icon" : "") +
          '">' +
          (this.options.iconCaret
            ? '<span class="' + this.options.iconCaret + ' pulldown" />'
            : "") +
          (this.options.iconRemove
            ? '<span class="' + this.options.iconRemove + ' remove" />'
    ...
    render: function (items) {
      ...
      items = $(items).map(function (i, item) {
        if (~that.renderLimit && i >= that.renderLimit) return;
    ...
    blur: function (e) {
      ...
      if (!this.selected && val !== "") {
        if (that.clearIfNoMatch) this.$element.val("");
